...
export default function CheckpointAIAnalysisPage() {
  ...
  const handleExecuteAction = async (
    ...
    try {
      const response = await backendClient.post(
        '/api/checkpoint/get-operation-guide',
        {
      ...
      >
        <div className="flex items-center gap-3 mb-2">
          <h1 className="text-3xl font-bold text-white">
            AI Cyber Security Analysis - Checkpoint
          </h1>
          {isLoading && (
        ...
        <p className="text-slate-400">
          基於 Checkpoint 安全數據的智能分析與建議 | 總計 {totalOpenIssues}{' '}
          次檢測，影響 {totalAffectedAssets} 個網址
        </p>
        {error && (
                    ...
                    {wafRisks
                      ...
                      .map((assessment) => (
                        ...
                        >
                          ...
                          {/* 🆕 受影響網址清單 */}
                          {assessment.affectedUrlList &&
                            assessment.affectedUrlList.length > 0 && (
                                  ...
                                  {assessment.affectedUrlList
                                    ...
                                    .map((url: string, idx: number) => (
                                      <Badge
                                        key={idx}
                                        variant="outline"
                                        className="text-xs font-mono bg-slate-800/50 text-slate-300 border-slate-600"
                                      >
                                        {url}
                                      </Badge>
                                    ))}
                          ...
                          <div className="mt-6 p-4 bg-cyan-900/20 border border-cyan-500/30 rounded-lg">
                            ...
                            <p className="text-slate-300 leading-relaxed text-sm whitespace-pre-wrap">
                              {assessment.aiInsight ||
                                `根據威脅情報分析，檢測到 ${assessment.openIssues} 次攻擊事件，共影響 ${assessment.affectedAssets} 個網址。建議立即採取防護措施並監控相關日誌。`}
                            </p>
                          </div>

                          {/* 🆕 TOP 攻擊來源 IP 區塊 */}
                          {assessment.topAttackers &&
                            assessment.topAttackers.length > 0 && (
                              <div className="mt-4 p-4 bg-slate-800/50 border border-orange-500/30 rounded-lg">
                                <div className="flex items-center gap-2 mb-3">
                                  ...
                                  <Badge
                                    variant="outline"
                                    className="ml-auto bg-orange-500/10 text-orange-400 border-orange-500/30 text-xs"
                                  >
                                      ...
                                      >
                                        ...
                                        <div className="flex items-center gap-4 text-sm">
                                          ...
                                          <Badge
                                            className={`
                                      ${
                                        attacker.behavior === 'IPS 觸發'
                                          ? 'bg-red-500/20 text-red-400 border-red-500/50'
                                          : attacker.behavior === '端口掃描'
                                ...
                                {/* 目標端口摘要 */}
                                {(assessment.topAttackers?.[0]?.targetPorts
                                  ?.length ?? 0) > 0 && (
                                  <div className="mt-3 p-2 bg-slate-900/30 rounded text-xs text-slate-400">
                                    ...
                                    <span className="ml-2 text-slate-300">
                                      {[
                                        ...new Set(
                                          assessment.topAttackers.flatMap(
                                            (a: any) => a.targetPorts || [],
                                          ),
                                        ),
                                      ]
