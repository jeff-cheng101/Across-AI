...
| VIOL_BROWSER | Unsupported browser | The system checks that the browser type and version is compatible with the application supporting the features required for rendering the pages correctly and securely. |
| VIOL_GRPC_METHOD | Illegal gRPC method | The system checks that the gRPC service method invoked matches one of the methods defined in the IDL file. The violation is triggered if the method does not appear there. |
| VIOL_GRPC_FORMAT | gRPC data does not comply with format settings | The system checks that the request contains gRPC content and complies with the various request limits within the defense configuration in the security policyâs gRPC profile. Enforces valid gRPC requests and protects the server from Protocol Buffers parser attacks. This violation is generated when a problem is detected in a gRPC request, generally checking the message according to boundaries such as the messageâs size and meta characters in parameter value. |
| VIOL_GRPC_MALFORMED | Malformed gRPC data | The system checks that the request contains gRPC content that is well-formed. Enforces parsable gRPC requests. |
| VIOL_BOT_CLIENT | Bot Client Detected | The system detects automated clients, and classifies them to Bot types. |
