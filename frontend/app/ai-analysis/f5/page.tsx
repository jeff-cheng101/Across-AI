...
export default function F5AIAnalysisPage() {
  ...
  const toggleItemsExpanded = (
    ...
    setItemsExpanded((prev) => ({
      ...
      [historyId]: {
        resolved:
          type === 'resolved'
            ? !prev[historyId]?.resolved
            : prev[historyId]?.resolved || false,
        unresolved:
          type === 'unresolved'
            ? !prev[historyId]?.unresolved
            : prev[historyId]?.unresolved || false,
      },
    }));
  };
  ...
  const categoryStats = {
    high: {
      ...
      openIssues: risksByCategory.high.reduce(
        (sum, r) => sum + (r.openIssues || 0),
        0,
      ),
      affectedAssets: risksByCategory.high.reduce(
    ...
    medium: {
      ...
      openIssues: risksByCategory.medium.reduce(
        (sum, r) => sum + (r.openIssues || 0),
        0,
      ),
      affectedAssets: risksByCategory.medium.reduce(
        (sum, r) => sum + (r.affectedAssets || 0),
        0,
      ),
    },
    low: {
      ...
      openIssues: risksByCategory.low.reduce(
        (sum, r) => sum + (r.openIssues || 0),
        0,
      ),
      affectedAssets: risksByCategory.low.reduce(
        (sum, r) => sum + (r.affectedAssets || 0),
        0,
      ),
    },
  ...
  const loadF5WAFRisks = async () => {
    ...
    try {
      ...
      const response = await backendClient.post('/api/f5/analyze-waf-risks', {
      ...
      if (data.metadata) {
        setAnalysisMetadata({
          ...
          analysisTimestamp:
            data.metadata.analysisTimestamp || new Date().toISOString(),
        });
      }
  ...
  const handleStartAnalysis = () => {
    ...
    const issuesResolvedCount = Math.floor(
      (affectedRisk?.openIssues || 0) * 0.35,
    );
    ...
    const generateProtectionMethod = (actionTitle: string): string => {
      if (
        actionTitle.includes('Threat Prevention') ||
        actionTitle.includes('威脅防護')
      )
        return 'Threat Prevention';
      if (actionTitle.includes('SandBlast') || actionTitle.includes('沙箱'))
        return 'SandBlast Zero-Day';
      if (actionTitle.includes('IPS') || actionTitle.includes('簽名'))
        return 'IPS 防護';
    ...
    const historyEntry: ExecutionHistory = {
      ...
      resolvedIssues: generateResolvedIssues(
        issuesResolvedCount,
        affectedRisk?.title || '',
      ),
    ...
    setExecutedActions((prev) =>
      new Set(prev).add(
        `${selectedAction?.issueId || ''}-${selectedAction?.title || ''}`,
      ),
    );
    // 這段結束
  ...
  const totalOpenIssues = wafRisks.reduce(
    (sum, risk) => sum + (risk.openIssues || 0),
    0,
  );
  const totalResolvedIssues = wafRisks.reduce(
    (sum, risk) => sum + (risk.resolvedIssues || 0),
    0,
  );
  const totalEvents = totalOpenIssues + totalResolvedIssues;
  const totalAffectedAssets = wafRisks.reduce(
    (sum, risk) => sum + (risk.affectedAssets || 0),
    0,
  );
  ...
  const handleExecuteAction = async (
    ...
    try {
      const response = await backendClient.post('/api/f5/get-operation-guide', {
  ...
  const confirmExecution = async () => {
    ...
    const generateProtectionMethod = (actionTitle: string): string => {
      if (
        actionTitle.includes('WAF') ||
        actionTitle.includes('防護') ||
        actionTitle.includes('簽名')
      )
        return 'WAF 防護策略';
      if (actionTitle.includes('速率') || actionTitle.includes('限制'))
        return '速率限制';
      if (actionTitle.includes('會話') || actionTitle.includes('Session'))
        return '會話保護';
      if (actionTitle.includes('訪問控制') || actionTitle.includes('URL'))
        return 'URL 訪問控制';
      return 'F5 安全規則';
    };
    ...
    setExecutedActions((prev) =>
      new Set(prev).add(`${selectedAction.issueId}-${selectedAction.title}`),
    );
              ...
              >
                <CardContent className="space-y-3 max-h-96 overflow-y-auto">
                  {history.map((historyItem) => {
                    const isResolvedExpanded =
                      itemsExpanded[historyItem.id]?.resolved || false;
                    const isUnresolvedExpanded =
                      itemsExpanded[historyItem.id]?.unresolved || false;
                    const totalResolved = historyItem.resolvedIssues.reduce(
                      (sum, item) => sum + item.count,
                      0,
                    );
                    const totalUnresolved = historyItem.unresolvedIssues.reduce(
                      (sum, item) => sum + item.count,
                      0,
                    );
                    const resolveRate = Math.round(
                      (totalResolved / (totalResolved + totalUnresolved)) * 100,
                    );
                      ...
                      >
                        ...
                        <div className="mt-4 space-y-2">
                          <div
                            ...
                            onClick={() =>
                              toggleItemsExpanded(historyItem.id, 'resolved')
                            }
                          >
                            <div className="flex items-center gap-2">
                              <CheckCircle className="w-4 h-4 text-green-400" />
                          ...
                          <AnimatePresence>
                            {isResolvedExpanded && (
                              ...
                              >
                                {historyItem.resolvedIssues.map(
                                  (issue, idx) => (
                                    <div
                                      key={idx}
                          ...
                          <div
                            ...
                            onClick={() =>
                              toggleItemsExpanded(historyItem.id, 'unresolved')
                            }
                          >
                            <div className="flex items-center gap-2">
                          ...
                          <AnimatePresence>
                            {isUnresolvedExpanded && (
                              <motion.div
                                ...
                                className="space-y-2 pl-4"
                              >
                                {historyItem.unresolvedIssues.map(
                                  (issue, idx) => (
                                    <div
                                      key={idx}
    ...
    <div className="min-h-screen bg-[#08131D] p-6">
      ...
      >
        ...
        <p className="text-slate-400">
          基於 F5 WAF 安全數據的智能分析與建議 | 總計 {totalOpenIssues}{' '}
          個開放問題，影響 {totalAffectedAssets} 個資產
        </p>
        {error && (
            ...
            <CardContent className="p-4">
              ...
              {analysisMetadata.timeRange.display?.start && (
                <div className="text-xs text-slate-400 space-y-0.5">
                  <div>
                    {formatDateTime(analysisMetadata.timeRange.display.start)}
                  </div>
                  <div className="text-center">至</div>
                  <div>
                    {formatDateTime(analysisMetadata.timeRange.display.end)}
                  </div>

                  {/* 顯示實際日誌時間範圍（如果與預期不同） */}
                  {analysisMetadata.timeRange.actual &&
                    analysisMetadata.timeRange.hasLogs && (
                      <div className="mt-2 pt-2 border-t border-slate-700/50">
                        ...
                        <div className="text-[10px]">
                          {formatDateTime(
                            analysisMetadata.timeRange.actual.start,
                          )}
                        </div>
                        <div className="text-center text-[10px]">至</div>
                        <div className="text-[10px]">
                          {formatDateTime(
                            analysisMetadata.timeRange.actual.end,
                          )}
                        </div>
                      </div>
                    )}
              ...
              {!analysisMetadata.timeRange.display &&
                analysisMetadata.timeRange.start && (
                  <div className="text-xs text-slate-400 space-y-0.5">
                    <div>
                      {formatDateTime(analysisMetadata.timeRange.start)}
                    </div>
                    <div className="text-center">至</div>
                        ...
                        customDateRange.end && (
                          <div className="p-2 bg-cyan-900/20 border border-cyan-500/30 rounded text-xs text-cyan-400 flex items-center gap-2">
                            ...
                            <span>
                              ...
                              至{' '}
                              {format(customDateRange.end, 'yyyy-MM-dd HH:mm')}
                            </span>
                          </div>
                        )}
              ...
              {error ? (
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div className="max-w-2xl">
                    ...
                    <p className="text-slate-400 text-base leading-relaxed">
                      選擇時間範圍後，點擊右上角「開始 AI 分析」按鈕，系統將使用
                      F5 多層次判斷模型分析 WAF 日誌並生成安全報告
                    </p>
                  </div>
              ...
              <div className="space-y-3 mt-4">
                ...
                <div className="p-3 bg-yellow-900/20 border border-yellow-500/30 rounded-lg">
                  ...
                  <div className="text-xs text-slate-300">
                    此操作預計將解決約{' '}
                    {Math.floor(
                      (wafRisks.find((r) => r.id === selectedAction?.issueId)
                        ?.openIssues || 0) * 0.3,
                    )}{' '}
                    個相關事件
                  </div>
                    ...
                    <div className="space-y-2 pt-3 border-t border-red-500/20">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-white font-semibold">
                          {categoryStats.high.openIssues}
                        </span>
                      </div>
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-slate-400">受影響資產</span>
                        <span className="text-white font-semibold">
                          {categoryStats.high.affectedAssets}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2 pt-3 border-t border-yellow-500/20">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-slate-400">開放問題</span>
                        <span className="text-white font-semibold">
                          {categoryStats.medium.openIssues}
                        </span>
                      </div>
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-slate-400">受影響資產</span>
                        <span className="text-white font-semibold">
                          {categoryStats.medium.affectedAssets}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2 pt-3 border-t border-blue-500/20">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-slate-400">開放問題</span>
                        <span className="text-white font-semibold">
                          {categoryStats.low.openIssues}
                        </span>
                      </div>
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-slate-400">受影響資產</span>
                        <span className="text-white font-semibold">
                          {categoryStats.low.affectedAssets}
                        </span>
                      </div>
                    </div>
                          ...
                          <div className="mt-6 p-4 bg-cyan-900/20 border border-cyan-500/30 rounded-lg">
                            ...
                            <p className="text-slate-300 leading-relaxed text-sm">
                              {assessment.aiInsight ||
                                `根據 F5 WAF 威脅情報分析，檢測到 ${assessment.openIssues} 次攻擊事件，共影響 ${assessment.affectedAssets} 個資產。建議立即採取 F5 Advanced WAF 防護措施並啟用學習模式，優先處理高風險端點。`}
                            </p>
                          </div>
                        </div>
                      ))}
